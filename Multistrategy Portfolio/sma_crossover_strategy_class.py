...
class sma_crossover_strategy():
    ...
    def __init__(self, tickers: list, lookback_window: int):
        ...
        # Backtest Strategy
        self.strategy_returns, self.portfolio_weights = self.run(self.close_returns, self.open_returns, self.lookback_window)
    ...
    def run(self, close_returns: pd.DataFrame, open_returns: pd.DataFrame, lookback_window: int):
        
        # Generate strategy
        strategy_returns = (open_returns.shift(-2).where((pt.cumulative_returns(close_returns) > pt.cumulative_returns(close_returns).rolling(lookback_window).mean())))

        # Get non-lagged positions
        strategy_positions = (open_returns.where((pt.cumulative_returns(close_returns) > pt.cumulative_returns(close_returns).rolling(lookback_window).mean())))
