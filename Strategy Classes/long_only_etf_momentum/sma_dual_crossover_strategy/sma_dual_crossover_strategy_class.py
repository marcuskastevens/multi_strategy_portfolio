...
class sma_dual_crossover_strategy():
    ...
    def run(self, close_returns: pd.DataFrame, open_returns: pd.DataFrame, lookback_window_slow: int, lookback_window_fast: int):
        ...
        # Generate strategy
        strategy_returns =  (open_returns.shift(-2).where(pt.cumulative_returns(close_returns).rolling(lookback_window_fast).mean() > pt.cumulative_returns(close_returns).rolling(lookback_window_slow).mean()))

        # Get non-lagged positions
        strategy_positions = (open_returns.where(pt.cumulative_returns(close_returns).rolling(lookback_window_fast).mean() > pt.cumulative_returns(close_returns).rolling(lookback_window_slow).mean()))
